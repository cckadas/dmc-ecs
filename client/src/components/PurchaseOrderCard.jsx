...
import { faTruckFast, faClock, faBoxesPacking, faCircleCheck } from '@fortawesome/free-solid-svg-icons'


export default function PurchaseOrderCard({ title, subtitle, summary, loading }) {
  ...
  const statuses = [
    {
      ...
      value: summary.sentToSupplier,
      icon: faTruckFast,
    },
    {
      label: 'Awaiting Delivery',
      value: summary.awaitingDelivery,
      icon: faClock,
    },
    {
      ...
      <div className=" flex flex-col gap-4 border-b border-gray-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#2D5A42] text-white">
            <FontAwesomeIcon icon={faTruckFast} className={`h-5 w-5 ${loading ? 'animate-pulse' : ''}`}/>
          </div>
          ...
          {statuses.map((status, index) => (
            <div key={status.label} className="relative">
              ...
              <div className="relative z-10 rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
                ...
                {/* STAGE NAME */}
                <p className="mt-4 text-sm font-semibold leading-5 text-gray-700">
                  {status.label}
                </p>
              </div>
            </div>
